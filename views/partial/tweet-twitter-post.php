<?php
    ...
    // Tweet
    $tweeted_by_tweet = $tweet['full_text'];
    ...
    $tweeted_tweet_media_url = '';
    $tweeted_tweet_media_url = $tweet['extended_entities']['media']['0']['media_url_https'];

    $tweeted_tweet_media_video_url = '';
    $tweeted_tweet_media_video_url = $tweet['extended_entities']['media']['0']['video_info']['variants'];

    if ($tweeted_tweet_media_video_url != '' ) {
        foreach($tweeted_tweet_media_video_url as $video) {
        ...
        $tweeted_tweet_media_video_url = $quality_video_url;
    }
    
    // SHOW ORIGINAL TWEET AUTHOR
    echo '<div class="tweet-author-wrap">';
        echo '<div class="tweet-author-img">';
            echo '<img width="75" src="'. $tweeted_by_profile_picture .'">';
        echo '</div>';

        echo '<div class="tweet-author-info">';
            echo '<div class="tweet-author-name">';
                echo '<a href="http://www.twitter.com/'. $tweeted_by_screen_name .'"><span>'. $tweeted_by_name .'</span></a>';
            echo '</div>';

            if ( $tweeted_by_user_verified == 1 ) {
    ...
    if ( $tweeted_tweet_media_url != '' ) {

        echo '<div class="tweet-img-wrap">';
            // echo '1';
            if ( $tweeted_tweet_media_video_url !== null ) {
                echo '<a class="play-video-link" href="'. $tweeted_tweet_media_video_url .'" data-lity-target="'. $tweeted_tweet_media_video_url .'" data-lity></a>';
                ...
                echo '<a class="play-video-link" href="'. $tweeted_tweet_media_url .'" data-lity-target="'. $tweeted_tweet_media_url .'" data-lity></a>';
            }
            echo '<img src="'. $tweeted_tweet_media_url .'" alt="twitter image">';
        echo '</div>';
        ...
        if ( $tweeted_media_type != '' ) {
            ...
            echo '<div class="retweet-text-wrap">';
                echo '<p>';
                    // The Regular Expression filter
                    $reg_exUrl = "/(http|https|ftp|ftps)\:\/\/[a-zA-Z0-9\-\.]+\.[a-zA-Z]{2,3}(\/\S*)?/";
                    $text = $tweeted_quote_text;
            ...
            $retweeted_tweeted_quote_text = $tweet['quoted_status']['text'];
            ...
            echo '<div class="retweet-text-wrap">';
                echo '<p>';
                    // The Regular Expression filter
                    $reg_exUrl = "/(http|https|ftp|ftps)\:\/\/[a-zA-Z0-9\-\.]+\.[a-zA-Z]{2,3}(\/\S*)?/";
                    $text = $retweeted_tweeted_quote_text;
            ...
            echo '<div class="retweet-text-wrap">';
                echo '<p>';
                    // The Regular Expression filter
                    $reg_exUrl = "/(http|https|ftp|ftps)\:\/\/[a-zA-Z0-9\-\.]+\.[a-zA-Z]{2,3}(\/\S*)?/";
                    $text = $retweeted_in_reply_quote_text;
                ...
                echo '</p>';
            echo '</div>';
        } else if ( $tweeted_tweet_exspand_url != '' ) {   
            echo '<div class="scrape-media">';
                echo getMetaData( $tweeted_tweet_exspand_url );
            echo '</div>';
        }  
    }
